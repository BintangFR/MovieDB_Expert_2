...
class TVShowsFavoriteAdapter(private val context: Context) :
    ListAdapter<DataModels, TVShowsFavoriteAdapter.ViewHolder>(DiffCallback()) {
    ...
    inner class ViewHolder(private val binding: ItemMoviesBinding, private val context: Context) :
        ...
        fun bind(dataModels: DataModels, userInterface: UserInterface?) {
            Glide.with(binding.root).load("${BuildConfig.IMAGE_URL}${dataModels.poster}")
